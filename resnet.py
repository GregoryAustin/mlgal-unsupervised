...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, encoder, num_classes=2): # used to b 10
        super(ResNet, self).__init__()      
        self.in_planes = 16

        self.encoder = encoder

        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1, bias=False) # NEW 
...
# END OF OLD VERSION 
    ...
    def forward(self, x):
        # out = self.encoder(x) TODO: use the encoder
...
def ResNet18(encoder):
    return ResNet(BasicBlock, [2,2,2,2], encoder, 2) # NEW 

def ResNet34(encoder):
    return ResNet(BasicBlock, [3,4,6,3], encoder, 2)
